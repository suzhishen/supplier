...
export class FastChangeOrderShowTree extends Component {
    setup() {
        ...
        onWillStart(async () => {
            this.state.orderRecord = await this.ormService.call(this.props.record.resModel, 'get_change_order_show_datas', [[this.props.record.resId]])
        })
        ...
        onWillUpdateProps(async (nextProps) => {
            ...
            if ((nextProps.record.resId !== this.props.record.resId && nextProps.record.resId > 0) || nextProps.readonly === true) {
                this.state.orderRecord = await this.ormService.call(this.props.record.resModel, 'get_change_order_show_datas', [[nextProps.record.resId]])
            }
        })
    }

    _unlinkchangeApplyOrderLineIds(unlink_ids) {
    ...
    _addchangeApplyOrderLineIds(add_ids) {
    ...
    async applychangeMaterialRequisitionBtn() {
    ...
    async change_update_order_detail(event){
        console.log(event)
        let self = this
        this.ormService.call('fast.blank_order_detail', 'change_update_order_detail', [[]], {
            'product_color_name': event.target.className.replace(/layui-btn layui-btn-fluid /g, ''),
        }).then(result => {
            console.log(result)
            self.env.model.load()
        })
...
FastChangeOrderShowTree.template = 'fast_supplier_synergy.change_order_show_tree'
registry.category("view_widgets").add("fast_order_center_change_order_show_tree", FastChangeOrderShowTree);